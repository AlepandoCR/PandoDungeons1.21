...
import battlePass.rewards.items.tools.EnchantedPickaxeReward;
import battlePass.rewards.items.tools.NetheriteSwordReward;
import battlePass.rewards.items.utility.BeaconReward;
...
public class PremiumRewardManager extends RewardManager {
    ...
    public void InitRewards(){
        new DiamondReward(plugin);
        new EmeraldBlockReward(plugin);
        new EmeraldBlockReward(plugin);
        new EnchantedPickaxeReward(plugin);
        new NetheriteSwordReward(plugin);
        new BeaconReward(plugin);
        new ElytraReward(plugin);
        new GoldenAppleReward(plugin);
        new TotemReward(plugin);
        new DiamondReward(plugin);
        new EmeraldBlockReward(plugin);
        new EmeraldBlockReward(plugin);
        new EnchantedPickaxeReward(plugin);
